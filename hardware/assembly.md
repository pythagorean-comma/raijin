...
| 33  | AGND     | Switch lamp (P4)                   |
| 36  | 3V3(OUT) | MOSFET module VCC (S5)             |
| 38  | GND      | Main ground from the GND bus (P10) |
| 39  | VSYS     | Main supply through the diode (P9) |

Pin 33 is the analogue ground, not a plain one. It works for the switch lamp, but pin 3 or pin 8 is a better home for 20 mA and both are nearer the switch.
...
Each strip gets its own pair of wires back to the box. Nothing joins one strip to the next.

That is fewer joints than chaining them, three instead of five, and every one of them is inside the box or at the strip itself rather than strung between two strips on the outside. It also means all three see the same voltage, so they light evenly. In a chain, strips two and three run on whatever is left after the drop through the ones in front, and at 5 V that shows.
